...
const Dashboard = () => {
  ...
  const fetchDashboardData = async (timeRange = selectedTimeRange) => {
    ...
    try {
      ...
      // Map UI timeRange to backend params
      const getRangeParams = (range) => {
        ...
        const period = range === '90d' ? 'weekly' : 'daily';
        return {
          period,
      ...
      const { period, startDate, endDate } = getRangeParams(timeRange);

      // Fetch all data in parallel with correct params
      const [
        statsRes,
        ordersRes,
        revenueRes,
        ...
        apiRequest(`/api/v1/admin/analytics/customers`, { token }),
      ]);
      ...
      setStats([
        {
          title: 'Total Revenue',
          value: `₹${(totalRevenue || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
          change: '+0%',
          trend: 'up',
          icon: IndianRupee,
          color: 'green',
        ...
        },
        {
          ...
          value: `₹${(avgOrderValue || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
          change: '+0%',
          trend: 'up',
          icon: Package,
          color: 'red',
...
setActivities((ordersRes.data.orders || []).slice(0, 5).map((order, idx) => {
  ...
  return {
    ...
    color: 'blue',
      ...
      const usersSeries = (customerRes.data.newCustomers || []).map(d => ({
        name: d._id,
        users: d.count || 0,
      }));

      const byName = new Map();
      [...revenueSeries, ...usersSeries].forEach(pt => {
        const existing = byName.get(pt.name) || { name: pt.name, sales: 0, revenue: 0, users: 0 };
        byName.set(pt.name, { ...existing, ...pt });
      });
      const merged = Array.from(byName.values()).sort((a, b) => (a.name > b.name ? 1 : -1));
      setChartData(merged);

      // Category distribution (pie)
      const categories = productRes.data.topCategories || [];
      setPieData(categories.map(c => ({
      ...
      setTopProducts(top.map(p => ({
        ...
        sales: p.totalSold || 0,
        revenue: `₹${(p.revenue || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
        trend: 'up'
      })));
  ...
  const getStatusColor = (status) => {
    ...
    if (s === 'processing' || s === 'packaging') return 'bg-yellow-100 text-yellow-800 border-yellow-200';
    if (s === 'out for delivery' || s === 'out_for_delivery' || s === 'shipped') return 'bg-blue-100 text-blue-800 border-blue-200';
    ...
    if (s === 'pending') return 'bg-gray-100 text-gray-800 border-gray-200';
