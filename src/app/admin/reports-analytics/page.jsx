    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 p-4 sm:p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 sm:mb-8">
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-800 mb-1 sm:mb-2">Reports & Analytics</h1>
          <p className="text-sm sm:text-base text-gray-600">Comprehensive insights into your farm marketplace performance</p>
        </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
            <div className="flex flex-col xs:flex-row gap-2 sm:gap-4 w-full sm:w-auto">
              <div className="flex items-center gap-2 w-full xs:w-auto">
                ...
                <select 
                  value={selectedTimeRange} 
                  onChange={(e) => setSelectedTimeRange(e.target.value)}
                  className="w-full xs:w-auto px-3 py-1 sm:px-4 sm:py-2 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                >
                  <option value="7d">Last 7 days</option>
                  <option value="30d">Last 30 days</option>
                  <option value="90d">Last 90 days</option>
            ...
            <div className="flex flex-wrap gap-2 w-full sm:w-auto justify-end">
              <button 
                onClick={() => handleExport('PDF')}
                disabled={isExporting}
                className="flex items-center gap-1 sm:gap-2 px-3 py-1 sm:px-4 sm:py-2 text-sm sm:text-base bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 transition-colors"
              >
              ...
              <button 
                onClick={() => handleExport('Excel')}
                disabled={isExporting}
                className="flex items-center gap-1 sm:gap-2 px-3 py-1 sm:px-4 sm:py-2 text-sm sm:text-base bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors"
              >
                <Download className="w-3 h-3 sm:w-4 sm:h-4" />
                Excel
              </button>
              <button 
                className="flex items-center gap-1 sm:gap-2 px-3 py-1 sm:px-4 sm:py-2 text-sm sm:text-base bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors" 
          ...
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 border-l-4 border-green-500">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-lg sm:text-2xl font-bold text-gray-800">₹{dashboard?.revenue?.total?.toLocaleString() || 0}</p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 border-l-4 border-blue-500">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-lg sm:text-2xl font-bold text-gray-800">{dashboard?.customers?.total?.toLocaleString() || 0}</p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 border-l-4 border-yellow-500">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-lg sm:text-2xl font-bold text-gray-800">{dashboard?.orders?.total?.toLocaleString() || 0}</p>
              </div>
              <div className="bg-yellow-100 p-2 sm:p-3 rounded-full">
                <ShoppingCart className="w-4 h-4 sm:w-6 sm:h-6 text-yellow-600" />
              </div>
            </div>
          </div>
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 border-l-4 border-purple-500">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-lg sm:text-2xl font-bold text-gray-800">{dashboard?.customers?.new?.toLocaleString() || 0}</p>
                <p className="text-purple-600 text-xs sm:text-sm mt-1">New signups</p>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 md:gap-8 mb-6 sm:mb-8">
          ...
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
            <h2 className="text-lg sm:text-xl font-semibold text-gray-800 mb-3 sm:mb-4">Sales & Revenue Trends</h2>
          ...
          {/* User Growth Chart */}
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
            <h2 className="text-lg sm:text-xl font-semibold text-gray-800 mb-3 sm:mb-4">User Growth Analytics</h2>
            <div className="h-[250px] sm:h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={userGrowthChartData}>
                  <CartesianGrid strokeDasharray="3 3" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
          {/* Order Trends Chart */}
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
            <h2 className="text-lg sm:text-xl font-semibold text-gray-800 mb-3 sm:mb-4">Order Trends by Category</h2>
            <div className="h-[250px] sm:h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={orderTrendsData}>
                  <CartesianGrid strokeDasharray="3 3" />
          ...
          {/* Category Distribution */}
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
            <h2 className="text-lg sm:text-xl font-semibold text-gray-800 mb-3 sm:mb-4">Sales Distribution</h2>
            <div className="h-[250px] sm:h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
        ...
        {isExporting && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-4 sm:p-6 flex items-center gap-3 sm:gap-4">
              <div className="animate-spin rounded-full h-6 w-6 sm:h-8 sm:w-8 border-b-2 border-green-600"></div>
              <span className="text-sm sm:text-base text-gray-700">Generating report...</span>
            </div>
          </div>
        )}
      </div>
